...
if ((Test-Path "${PSScriptRoot}\..\..\..\Support") -eq 1)
{
    $supportPath = Resolve-Path (Join-Path $PSScriptRoot '../../../Support')
    Write-Output "Support path found: ${supportPath}"
}
else 
{
    Write-Output "Support path not found, please ensure Support folder from Archicad API is located 3 folders back from template!"
    exit
}
...
& "${supportPath}\Tools\Win\AddOn Management\AddOnAdmin.exe"
